...
The zero-award agent-service contract extension is [upgraded and finalized on Devnet](docs/deployment/deployment.community.devnet.json). Independent [chain verification](docs/deployment/personal-agent.chain.validation.json) passed 36 checks covering all nine records from the hosted watcher journey: agent start and human return, the human guardian's 25-point/10-reputation contribution, and one free banner. The actual deployed binary and append transactions were verified. New journeys still publish asynchronously; a pending record is not a confirmed receipt. See the [release evidence](docs/deployment/personal-agent.release.validation.json).
...
Companionship remains accessible without payment, staking, or a token balance. The [gratitude rules](docs/GRATITUDE_MECHANISM.md) allow optional free banners after closure without adding points or reputation. Financial tips and transferable tokens are deferred; translation is not integrated. The site is publicly accessible. The account update provides **Create account** with a display name, username and password, and **Sign in** for return visits to the same account. No site invitation, email address, or wallet is required. See [Demo accounts](docs/DEMO_ACCOUNTS.md) for account setup and [Validation](docs/VALIDATION.md) for release evidence.
...
Use separate browser profiles for the rider and two guardians, with a different registered account in each. Participants need no wallet or SOL: accept the public-record notice, create a journey, share its invite, have a guardian apply, then approve them from the rider session. Existing authenticated guests can save a username and password through **Account & settings → Save my account**, preserving their account and history. New real journeys automatically queue minimal history and recognition for sponsored publication on Solana. Only approved participants can see route, location, and conversation. Request a relay to recruit a replacement. Confirm arrival to end monitoring.
...
1. Each participant opens **Account & settings**, links a Solana wallet, and saves the recovery code. Select Devnet in the wallet and obtain test SOL for transaction fees.
...
| Area | Implementation |
| --- | --- |
| Accounts | Username/password registration and sign-in, same-account guest upgrades, optional verified wallet binding, expiring/revocable sessions, wallet rotation, single-use recovery codes |
...
Public access was enabled on September 20, 2026. The earlier [anonymous HTTP acceptance](docs/deployment/public-access.validation.json) verified the homepage, two independent guest registrations through the frontend proxy, authenticated account access, and cleanup without a privileged site credential. That receipt predates username/password registration and does not validate it. Public access changes who can enter the site; private journeys still require approved participation. The [simplified frontend](docs/FRONTEND_SIMPLIFICATION.md) puts active journey controls beside conversation, folds ended-journey history, and combines account actions in an avatar menu. The [appreciation wall and honor cabinet](docs/COMMUNITY_HONORS.md) remain public to members, with compact previews and complete receipt access. See [DEMO.md](docs/DEMO.md) to demonstrate recognition using separate rider and guardian identities.
...
**Account & settings** supports saving an existing guest account, password changes, optional wallet linking/recovery, wallet replacement, session revocation, export, and deletion. Registered accounts can sign in again after logout or from another browser or device. Sessions remain tab-specific, and changing a password replaces the current session and revokes other sessions. There is no email password-reset provider. A display name alone cannot recover a logged-out, unbound guest. Changing an account wallet does not transfer existing chain authority: outstanding journeys still require their original signing wallet.

Closed private journeys expire after 30 days (7 days for simulated demos). Riders may erase a closed journey earlier. Public chain records and minimal anti-replay/ownership tombstones remain. Restores preserve deletion protections and do not restart old active journeys. Current backup snapshots exclude username/password credentials, sessions and recovery credentials: restoring business data does not restore password-only account access. Wallet-linked accounts can sign in after the corresponding wallet registry is restored. See [DATA_POLICY.md](docs/DATA_POLICY.md).
