...
The [personal-agent protocol](PERSONAL_AGENT_PROTOCOL.md) adds an owner-operated runtime outside Cloudflare. The volunteer's Codex watcher or local STDIO MCP adapter calls the Worker using one revocable journey capability. `TripRoom` owns both participants' approval, hashed capability verification, current jobs, readiness/deadlines and atomic action receipts. The runtime sees minimized approved context; it has no account-session, contact, wallet, signing-key or reward authority. Personal model authentication stays on the owner's computer. The platform OpenAI API remains disabled.

Validated proposals reuse the existing semantic policy and canonical tools. The first accepted proposal establishes agent coverage; heartbeat and response deadlines separately expose lost connectivity or stalled reasoning. Human return revokes the capability and discards pending work. Named private service history is distinct from human check-ins. Only minimal service transitions enter the community publisher as zero-award `agent_service` records; independent finality is required before the interface calls them chain confirmed. See [validation evidence](VALIDATION.md) for the current deployment boundary.
