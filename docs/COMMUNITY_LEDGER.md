...
The kind 9 program upgrade is finalized on Devnet, with the validated binary independently matched to deployed program data. The [deployment receipt](deployment/deployment.community.devnet.json) records that upgrade; the separate [personal-agent chain report](deployment/personal-agent.chain.validation.json) records the currently verified journey receipts. A deployed program does not itself confirm every queued record. Publication remains ordered: a record awaiting publication or retry holds later records in that journey, and the UI must not show them as chain confirmed. Existing reward rule 1 and prior account layouts remain compatible.
...
The community program is deployed on Solana Devnet at [`7vuz78V9Tu53Bpt3iXHc37VSDBmRSYxcEguWewR9LXzm`](https://explorer.solana.com/address/7vuz78V9Tu53Bpt3iXHc37VSDBmRSYxcEguWewR9LXzm?cluster=devnet). Its personal-agent extension upgrade finalized at slot **501236996**. The [deployment receipt](deployment/deployment.community.devnet.json) verifies the finalized upgrade transaction, executable, upgrade authority, and exact validated binary hash. The sponsor was replenished with 0.25 Devnet SOL; that funding is a recorded top-up, not a guarantee of future publisher availability. Dedicated issuer and sponsor identities are initialized, and the Worker publisher is configured. Independent business-record verification is reported separately in the [personal-agent chain report](deployment/personal-agent.chain.validation.json).

The [personal-agent chain verification](deployment/personal-agent.chain.validation.json) passed **36 checks** for the hosted watcher acceptance journey. All **nine records** were independently decoded from finalized program-owned accounts and matched to finalized append transactions. Its two rule-version-2 service records describe agent start and human return, both with zero points and reputation. The human guardian retained two human check-ins, one 25-point / 10-reputation allocation, and one free companionship banner. This establishes that journey's publication and reward separation, not every current or future journey's finality. The read-only verifier made no model calls or external notifications.
