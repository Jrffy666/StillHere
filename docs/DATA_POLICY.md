...
This release does not use an official Uber order API or promise emergency dispatch. Ride details are shared by users. The OpenAI adapter is implemented but disabled. A key alone does not activate processing: server enablement, eligible journey state, budgets, and current v2 rider consent are required. Only current consenting participants' messages enter the minimized snapshot. Saved rider concerns, timestamps and limited journey status may also be included; personal information in free text can remain despite heuristic redaction. The request sets `store: false`, which does not guarantee zero provider retention. Revocation prevents future processing and invalidates pending work but cannot recall a transmitted request. Validated model assessments and source snapshots remain private journey data and follow the existing access, retention and deletion controls. See [OPENAI_INTEGRATION.md](OPENAI_INTEGRATION.md) for the data boundary and provider retention reference.

Voice and outbound notifications remain separately configured optional services. Participants need no provider credential or wallet for ordinary human guarding; sponsored community publication requires the operator's issuer, sponsor and Devnet funding. Community reports and fixed reward pools reduce some abuse but do not establish real-world identity or prevent all collusion.
