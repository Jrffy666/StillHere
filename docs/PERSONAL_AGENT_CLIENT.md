...
Use the actual saved connection path. For a file in the current project folder, use `--connection ".\stillhere-agent-connection.json"`; browser downloads can also have numbered filenames such as `stillhere-agent-connection (1).json`. Enter the complete command at one PowerShell prompt. A visual line wrap is not another command, and the optional limit flags cannot run alone. `STILLHERE_INPUT_FILE_NOT_FOUND` means the supplied path is missing or a parent is not a directory; `STILLHERE_INPUT_FILE_UNREADABLE` means access was denied. These errors occur before connecting to the journey or invoking a model. Correct the path or permissions; if the connection has since expired, request fresh participant approval as required by the app and download a new connection. Do not share the file contents.
