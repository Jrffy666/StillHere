...
Run `npm run test:personal-agent` for client/runner/MCP boundary tests and `npm run test:codex-demo` for the existing isolated CLI regression suite. These tests use mocked inference and a loopback HTTP server; the native CLI regression also uses a loopback fake provider and performs no real model call. The current client suite passes 30 tests, including an actual STDIO process and the official TypeScript MCP SDK 1.30.0 performing initialization, tool discovery and a tool call with standard metadata. The SDK interoperability test skips only when that optional development dependency is absent.

The SDK acceptance is separate from a native Codex MCP demonstration. A bounded offline Codex 0.155.1 probe recognized the configured server but exposed no tools in its restricted fake-provider request; it did not establish native MCP execution. No saved Codex configuration or authentication was changed. The primary verified Codex runtime is the bounded `watch` integration. Interactive Codex MCP discovery and tool invocation should be checked in the intended presentation client before claiming that demonstration path is ready.
