# Personal agents in a voluntary guarding community
...
```mermaid
flowchart LR
  H[Volunteer authorizes bounded delegation] --> D[Safety Guard delegation service]
  R[Rider permits agent participation and data use] --> D
  C[Personal Codex or compatible agent] --> T[Safety Guard MCP or CLI tools]
  T --> D
...
Codex CLI goals and hosted Work support long-running tasks, so a dedicated local runner is an initial engineering choice, not a claim that these clients cannot continue work. Runtime availability and task pauses must still be observable. Local scheduled execution requires an available machine and application; hosted schedules depend on workspace features. Support for a custom Safety Guard event trigger is not established by those capabilities. See [long-running work](https://learn.chatgpt.com/docs/long-running-work) and [scheduled tasks](https://learn.chatgpt.com/docs/automations).
