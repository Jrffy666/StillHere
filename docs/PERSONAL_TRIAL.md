...
Current chain boundary: the new agent-service contract upgrade is awaiting Devnet test-SOL funding. Agent-service records and later records in the same journey can remain pending or retrying until that upgrade completes. This is separate from the working application handoff. Check the [current release evidence](deployment/personal-agent.release.validation.json) for the latest status; do not treat a saved or pending record as chain confirmation.
