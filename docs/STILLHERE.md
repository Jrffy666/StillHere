...
Use only fictional participants and synthetic journey details. The personal-agent flow is implemented; this expanded sequence is a proposed rehearsal, not evidence that every branch has been exercised with a real model.
...
2. **Ask for a bounded handoff.** Alex tells their personal agent, "I need to sleep for twenty minutes. Continue accompanying Sam and request a human if they need one." Alex submits the named-agent request and processing consent in the application; Sam approves that exact delegation with their own consent. Both consents authorize the journey-scoped capability. Runtime acceptance alone leaves coverage connecting; the first valid assessment activates coverage.
...
The community record distinguishes human guarding from automated support. The implemented contract extension defines zero-award `agent_service` events using minimal references and timestamps. Application journaling, contract deployment, and finalized receipts are separate checkpoints; the [chain validation evidence](deployment/personal-agent.chain.validation.json) governs publication status. A built extension or successful model response does not establish chain finality. Chat messages, precise routes, contacts, credentials and model prompts remain off chain.

The extension upgrade is finalized on Devnet. Separate [chain verification](deployment/personal-agent.chain.validation.json) passed 36 checks for the hosted watcher acceptance journey: all nine receipts and their append transactions were finalized, agent start and human return awarded no points, and the human guardian received one 25-point / 10-reputation allocation and one free banner. This verified journey does not establish finality for every other journey or broader model reliability.

The personal-agent implementation preserves the human contribution and free-banner rules. Agent actions do not add human check-ins or contribution points. Separate recognition for agent service needs an explicit later product decision. A banner remains a voluntary thank-you, not a price for companionship or proof of someone's character.

## What exists and what comes next

The current application contains human assignment and relay, named guardian-agent requests, rider consent, journey-scoped capabilities, source validation, execution receipts, unresolved concerns, truthful coverage status, and separate agent service history. The backend and interface are deployed. The [release evidence](deployment/personal-agent.release.validation.json) records 23 hosted acceptance checks with one real Codex assessment; the separate [published-browser run](deployment/personal-agent.browser.validation.json) passed 12 checks with another real Codex assessment, human return, capability revocation, arrival, and a free banner. These bounded runs do not establish broad model quality or unattended overnight availability.

The owner-operated Codex watcher and local STDIO MCP bridge are implemented. The [MCP interoperability receipt](deployment/personal-agent.mcp.validation.json) records 31 passing client tests, official SDK discovery and calls, and a native Codex/bridge check against a scripted loopback provider with no real model call. The watcher is the demonstrated real-model runtime; the MCP regression establishes protocol interoperability. Neither installs an always-running service or keeps a sleeping computer awake. The hosted OpenAI API remains disabled.

The next product step is the owner's [personal trial](PERSONAL_TRIAL.md), followed by broader scenario, failure, and sustained-availability measurements. Presentation scripting and visual redesign follow that feedback. Contract upgrade and independent receipt verification have their own chain evidence; application and browser acceptance must not be used as substitutes.
