...
The current [offline assistance boundary](AGENT_HARNESS.md) is published as frontend version 11 and production Worker version `81eeaaa5-5e46-4ebe-b8d6-8ea9aecb3376`, with the existing owner/demo-viewer access. It adds rider preferences, separate escalation authority, private retained concerns, server-rendered evidence, context-first tools, abortable provider calls, renewed leases, and persisted budgets. OpenAI and external notifications remain disabled. No chain program was changed.
