...
The production Worker is deployed at [safety-guard-api-production.2012044zj.workers.dev](https://safety-guard-api-production.2012044zj.workers.dev/api/ready); its version is recorded in the [hosting receipt](deployment/hosting.production.json). Its readiness endpoint returns HTTP 200 with `ready: true` and `environment: production`. The frontend is published with owner-only access at [safety-guard-htn2026.klavander56.chatgpt.site](https://safety-guard-htn2026.klavander56.chatgpt.site). The Worker signing domain, site origin, and allowed origins use this actual published domain.
