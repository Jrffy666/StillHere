...
The current [manual local Codex demonstration](CODEX_DEMO.md) is published as frontend version 14 and production Worker version `5e2ffd3a-d4c9-4694-9761-8c7331533d73`, with the existing owner/demo-viewer access. All **328 Worker tests across 20 files**, **24 local runner tests**, eight proxy tests, 13 journey-interface tests, and 14 honor tests passed. Worker source/test and frontend type checks, frontend lint and production build passed. The runner tests include a real native CLI against a loopback fake provider, with no model call in that regression test.
