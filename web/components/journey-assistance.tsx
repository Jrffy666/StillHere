...
export function JourneyAssistance({trip,token,viewerId,busy,act}:{trip:Trip;token:string;viewerId:string;busy:boolean;act:(body:ActionBody)=>Promise<void>}) {
  ...
  return <details className="simple-details journey-assistance">
    ...
    <p className="simple-note">{trip.liveAiAvailable?'OpenAI API assistance is available with participant consent.':'Hosted OpenAI API assistance is switched off. Offline reminders remain available.'} Personal-agent handoffs use their own permission and runtime in the handoff panel. Human companions remain responsible for their own check-ins.</p>
    ...
    <details><summary>Manual Codex protocol test</summary><CodexDemo key={`${trip.id}:${trip.codexDemo?.id??''}`} trip={trip} token={token} viewerId={viewerId} busy={busy||saving}/></details>
