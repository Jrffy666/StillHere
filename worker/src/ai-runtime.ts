...
import { renderSemanticQuestion } from './agent-semantic';
...
export function canUseLiveAi(env:WorkerEnv,trip:Trip):boolean {
  return openAiAvailable(env)&&!isClosed(trip)&&trip.guardMode==='ai'&&assistanceEnabled(trip)
    ...
    &&participantAiConsent(trip,trip.rider.id)&&trip.escalation?.cause!=='explicit_help';
}
...
export function nextSemanticDecision(run:AgentRun,context:AgentContext):AgentDecision {
  ...
  if(assessment.requestRelay&&!context.relayOpen&&['takeover','rider-message'].includes(run.trigger.kind)&&!used('request_human_relay'))return {type:'tool',call:{name:'request_human_relay',arguments:{reason:'The model proposed human coverage from current journey evidence. Rider approval is still required.'}}};
